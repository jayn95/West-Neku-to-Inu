    ...
    if(isset($_POST['petID']) && isset($_POST['liked'])) {
        ...
        if (isset($_SESSION['username'])) {
            ...
            if ($userRow) {
                ...
                $sql = "SELECT * FROM post_reactions WHERE userID = $userID AND petID = $petID";
                $result = mysqli_query($db, $sql);

                if(mysqli_num_rows($result) > 0) {
                    // User has already reacted, update the existing reaction
                    $row = mysqli_fetch_assoc($result);
                    $postReactID = $row['postReactID'];

                    if($liked) {
                        // User liked the animal, update the reaction
                        $sql_update = "UPDATE post_reactions SET liked = 1 WHERE postReactID = $postReactID";
                        mysqli_query($db, $sql_update);
                        echo 'liked';
                    } else {
                        // User unliked the animal, delete the reaction
                        $sql_delete = "DELETE FROM post_reactions WHERE postReactID = $postReactID";
                    ...
                    if($liked) {
                        ...
                        $sql_insert = "INSERT INTO post_reactions (userID, petID, liked, time_reacted) VALUES ($userID, $petID, 1, NOW())";
